...
void* ldopen(const char* filename, int flags) {
	...
	if (result == NULL) {
		loader_printf(LOG_ERR, "ldopen failed\n");
		return NULL;
	}
	return result;
}
...
void* ldsym(void* handle, const char* symbol) {
	...
	if (handle == NULL) {
		loader_printf(LOG_ERR, "dlsym library handle is null\n");
		return NULL;
	}
	if (symbol == NULL) {
		loader_printf(LOG_ERR, "dlsym symbol name is null\n");
		return NULL;
	}
	...
	if (sym != NULL) {
		...
		loader_printf(LOG_DBG, "symbol %s found but not global\n", symbol);
		return NULL;
	} else {
		loader_printf(LOG_DBG, "undefined symbol %s\n", symbol);
		return NULL;
	}
}
...
// in the strtab with a \0 to make iterating through the symtab easier.
		...
		ELF32_SYM_INITIALIZER( 0, &ldopen, 1),
		ELF32_SYM_INITIALIZER( 7, &ldclose, 1),
		ELF32_SYM_INITIALIZER(15, &ldsym, 1),
		ELF32_SYM_INITIALIZER(21, &lderror, 1),
