...
ElfReader::ElfReader(const char* name, int fd, int offset) :
		name_(name), fd_(fd), phdr_num_(0), phdr_mmap_(NULL), phdr_table_(NULL), phdr_size_(
				0), load_start_(NULL), load_size_(0), load_bias_(0), loaded_phdr_(
		NULL), bmap_(true), map_offset_(offset) {
}

ElfReader::ElfReader(const char* name, int fd, void* base, size_t size,
		bool bmap) :
		name_(name), fd_(fd), phdr_num_(0), phdr_mmap_(NULL), phdr_table_(NULL), phdr_size_(
				0), load_bias_(0), loaded_phdr_(NULL), load_start_(base), load_size_(
				size), bmap_(bmap), map_offset_(0) {
}
...
bool ElfReader::ReadElfHeader() {
	...
	if (rc < 0) {
		loader_printf(LOG_ERR, "can't read file \"%s\": %s\n", name_,
				strerror(errno));
		return false;
	}
	if (rc != sizeof(header_)) {
		loader_printf(LOG_ERR, "\"%s\" is too small to be an ELF executable\n",
				name_);
		return false;
	}
...
bool ElfReader::VerifyElfHeader() {
	if (header_.e_ident[EI_MAG0] != ELFMAG0
		...
		loader_printf(LOG_DBG, "\"%s\" has bad ELF magic %x\n", name_,*(unsigned int*)header_.e_ident);
		return false;
	}

	if (header_.e_ident[EI_CLASS] != ELFCLASS32) {
		loader_printf(LOG_DBG, "\"%s\" not 32-bit: %d\n", name_,
				header_.e_ident[EI_CLASS]);
		return false;
	}
	if (header_.e_ident[EI_DATA] != ELFDATA2LSB) {
		loader_printf(LOG_DBG, "\"%s\" not little-endian: %d\n", name_,
				header_.e_ident[EI_DATA]);
		return false;
	}
/*
	if (header_.e_type != ET_DYN) {
		loader_printf(LOG_DBG, "\"%s\" has unexpected e_type: %d\n", name_,
				header_.e_type);
		return false;
	}*/

	if (header_.e_version != EV_CURRENT) {
		loader_printf(LOG_DBG, "\"%s\" has unexpected e_version: %d\n", name_,
				header_.e_version);
		return false;
	}
	...
	EM_386
		...
		loader_printf(LOG_DBG, "\"%s\" has unexpected e_machine: %d", name_,
				header_.e_machine);
		return false;
	}
...
bool ElfReader::ReadProgramHeader() {
	...
	if (phdr_num_ < 1 || phdr_num_ > 65536 / sizeof(Elf32_Phdr)) {
		loader_printf(LOG_DBG, "\"%s\" has invalid e_phnum: %d", name_,
				phdr_num_);
		return false;
	}

	Elf32_Addr page_min = PAGE_START(header_.e_phoff);
	Elf32_Addr page_max = PAGE_END(
			header_.e_phoff + (phdr_num_ * sizeof(Elf32_Phdr)));
	...
	if (!bmap_) {
		mmap_result = malloc(load_size_);
		if (mmap_result) {
			lseek(fd_, load_start_, SEEK_SET);
			read(fd_, mmap_result, load_size_);
			
		}
	} else {
		if (map_offset_) {
			mmap_result = malloc(phdr_size_);
			if (mmap_result) {
				lseek(fd_, map_offset_, SEEK_SET);
				read(fd_, mmap_result, phdr_size_);
	
			}
		} else {
			mmap_result = mmap(NULL, phdr_size_, PROT_READ, MAP_PRIVATE, fd_,
					page_min);
			if (mmap_result == MAP_FAILED) {
				loader_printf(LOG_DBG, "\"%s\" phdr mmap failed: %s", name_,
						strerror(errno));
				return false;
			}
		}
 ...
 */
size_t phdr_table_get_load_size(const Elf32_Phdr* phdr_table, size_t phdr_count,
...
bool ElfReader::ReserveAddressSpace() {
	...
	if (load_size_ == 0) {
		loader_printf(LOG_DBG, "\"%s\" has no loadable segments", name_);
		return false;
	}
	...
	if (!bmap_) {
		...
		if (map_offset_) {
			start = mmap(NULL, load_size_, PROT_READ | PROT_WRITE | PROT_EXEC,
					MAP_PRIVATE | MAP_ANONYMOUS, 0, 0);
			loader_printf(LOG_DBG, "memory load base at @ %x\n", start);

			if (start == 0) {
				loader_printf(LOG_DBG,
						"couldn't mmap %d bytes memory for \"%s\"\n",
						load_size_, name_);
				return false;
			}
		} else {
			start = mmap(addr, load_size_, PROT_NONE, mmap_flags, -1,
					map_offset_);
			if (start == MAP_FAILED) {
				loader_printf(LOG_DBG,
						"couldn't reserve %d bytes of address space for \"%s\"\n",
						load_size_, name_);
				return false;
			}
		}
...
bool ElfReader::LoadSegments() {
	for (size_t i = 0; i < phdr_num_; ++i) {
		...
		if (!bmap_) {
			...
			if (map_offset_) {
				lseek(fd_, map_offset_ + file_start, SEEK_SET);
				read(fd_, seg_page_start, file_length);
				...
				if (file_length != 0) {
					...
					if (seg_addr == MAP_FAILED) {
						loader_printf(LOG_DBG,
								"couldn't map \"%s\" segment %d: %s", name_, i,
								strerror(errno));
						return false;
					}
				}
			...
			if (map_offset_ == 0) {
				if (seg_page_end > seg_file_end) {
					...
					if (zeromap == MAP_FAILED) {
						loader_printf(LOG_DBG,
								"couldn't zero fill \"%s\" gap: %s", name_,
								strerror(errno));
						return false;
					}
				}
 ...
 */
static int _phdr_table_set_load_prot(const Elf32_Phdr* phdr_table,
		int phdr_count, Elf32_Addr load_bias, int extra_prot_flags) {
 ...
 */
int phdr_table_protect_segments(const Elf32_Phdr* phdr_table, int phdr_count,
 ...
 *   0 on error, -1 on failure (error code in errno).
 */
int phdr_table_unprotect_segments(const Elf32_Phdr* phdr_table, int phdr_count,
		Elf32_Addr load_bias) {
 ...
 *   0 on error, -1 on failure (error code in errno).
 */
int phdr_table_protect_gnu_relro(const Elf32_Phdr* phdr_table, int phdr_count,
		Elf32_Addr load_bias) {
		...
		ElfReader* reader)
{
	...
	for (phdr = phdr_table; phdr < phdr_limit; phdr++) {
		...
		if(reader && reader->map_offset_)
		{
			*arm_exidx = (Elf32_Addr*)malloc(phdr->p_memsz);
			if(*arm_exidx)
			{
				lseek(reader->fd_,reader->map_offset_+phdr->p_offset,SEEK_SET);
				read(reader->fd_,*arm_exidx,phdr->p_memsz);
				if(reader->map_offset_)
				  ;
...
void phdr_table_get_dynamic_section(const Elf32_Phdr* phdr_table,
		int phdr_count, Elf32_Addr load_bias, Elf32_Dyn** dynamic,
		size_t* dynamic_count, Elf32_Word* dynamic_flags, ElfReader* reader) {
	const Elf32_Phdr* phdr = phdr_table;
	const Elf32_Phdr* phdr_limit = phdr + phdr_count;
	...
	for (phdr = phdr_table; phdr < phdr_limit; phdr++) {
		...
		if (reader && reader->map_offset_) {
			*dynamic = (Elf32_Dyn*) malloc(phdr->p_memsz);
			if (*dynamic) {
				lseek(reader->fd_, reader->map_offset_ + phdr->p_offset,
				SEEK_SET);
				read(reader->fd_, *dynamic, phdr->p_memsz);
				if(reader->map_offset_)
           ;
			}
		} else {
...
bool ElfReader::FindPhdr() {
	...
	loader_printf(LOG_DBG, "can't find loaded phdr for \"%s\"", name_);
	return false;
}
...
bool ElfReader::CheckPhdr(Elf32_Addr loaded) {
	...
	loader_printf(LOG_DBG, "\"%s\" loaded phdr %x not in loadable segment",
			name_, loaded);
	return false;
}
